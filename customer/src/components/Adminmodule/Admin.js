      ...
      <table >
        ...
        <tbody>{
          datas.map((value, index) => (
            <>
              <tr>
                <td className="border border-dark">{value.customer_id} </td>
                ...
                <td className="border border-dark d-flex">
                  <Link to={`/singledata/${value. customer_id}`} className="ms-3 nav-link text-info" ><Eye /></Link>
                  <Link to={`/edits/${value.customer_id}`} className="ms-3 nav-link" ><PencilSquare /></Link>

                  <button className="ms-3 nav-link text-danger" onClick={() => { del_data(value.customer_id) }} ><Trash /></button>
                </td>
              </tr>
